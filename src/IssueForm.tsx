...
import useTiDBVersions from "./hooks/useTiDBVersions";
...
const IssueForm: React.FC<{}> = () => {
  ...
  const { repoVersions, fetchTiDBVersions } = useTiDBVersions();
  ...
  const handleCreate = React.useCallback(() => {
    ...
    const label =
      issueType === "feature" ? "type%2Ffeature-request" : "type%2Fbug";
    localStorage.clear();

    // Hacking to test-dev.
    window.open(
      `https://github.com/ti-community-infra/${repo}/issues/new?title=${title}&body=${body}&labels=${label}`
    );
  }, []);

  React.useEffect(() => {
    fetchTiDBVersions(params.repo);
    bindModalHandler();
    restoreValues();
  }, []);
          ...
          <Col span={11}>
            ...
            >
              <Select onChange={handleRepoChange}>
                <Option value="test-dev">TiDB</Option>
              </Select>
            </Form.Item>
          </Col>
